...
- Clone [PLACEHOLDER?] this repository to your local machine. Navigate to the `Unity-Robotics-Hub/tutorials/pick_and_place/UnityAssets` directory. Copy all of the subfolders in UnityAssets (Environment, Materials, Plugins, etc.), and paste them into the Assets folder of your newly made Unity project.
...
> Note: PLACEHOLDER The world-space origin of the robot is defined in its URDF file. In this sample, we have assigned it to sit on top of the table, which is at `(0, 0.63, 0)` in Unity coordinates.
  ```xml
...
- The UR3 should now be in the scene, but under the table. Select the `niryo_one` object and set its position to `(0, 0.63, 0)` to place it on top of the table to match the URDF definition.
