...
* **Action**: Select the `Cube` GameObject and in the _**Inspector**_ tab, click on the _**Add Component**_ button.

* **Action**: Start typing `Labeling` in the search bar that appears, until the `Labeling` script is found, with a **#** icon to the left and double click on it. 

* **Action**: Press the **Add New Label** button and change `New Label` to `cube_position`. Then, click on `Add to Label Config...` and below `Other Label Configs in Project` there should be `IdLabelConfig`. Click on `Add Label` and then press the red X to close the window. 
...
* **Action**: Select the `Simulation Scenario` GameObject and in the _**Inspector**_ tab, click on the _**Add Component**_ button. Start typing `Fixed Length Scenario` in the search bar that appears, until the `Fixed Length Scenario` script is found, with a **#** icon to the left. Then double click on it. 
...
* **Action**: In the _**Project**_ tab, right-click on the **Scripts** folder and select `Create -> C# Script`. Name your new script file `YRotationRandomizer`.

* **Action**: Create another script and name it `YRotationRandomizerTag`.

* **Action**: Double-click `YRotationRandomizer.cs` to open it in _**Visual Studio**_.
...
* **Action**: Open `YRotationRandomizerTag.cs` and replace its contents with the code below:
...
* **Action**: Add `YRotationRandomizer` to the list of Randomizers in SimulationScenario (see gif after the next action). You will notice that you can adjust the distribution here, as mentioned above.

<p align="center">
<img src="Images/2_y_rotation_randomizer.png" height=100/>
</p>

* **Action**: Select the `Cube` GameObject and in the _**Inspector**_ tab, add a `YRotationRandomizerTag` component. 
...
* **Action**: Select the `Simulation Scenario` GameObject, and do the following:
...
* **Action**: Now we need to add the RandomizerTag to the cube. 
...
* **Action**: Select the `Simulation Scenario` GameObject and in the _**Inspector**_ tab, on the `Fixed Length Scenario` component, click on `Add Randomizer` and start typing `LightRandomizer`. 
